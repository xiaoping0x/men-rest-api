...
test('POST /users 201 (master)', async () => {
  const { status, body } = await request(app())
    .post(apiRoot)
    .send({
      access_token: masterKey, email: 'd@d.com', password: '123456', role: 'user',
    });
  expect(status).toBe(201);
  expect(typeof body).toBe('object');
  expect(typeof body.user).toBe('object');
...
test('POST /users 201 (master)', async () => {
  const { status, body } = await request(app())
    .post(apiRoot)
    .send({
      access_token: masterKey, email: 'd@d.com', password: '123456', role: 'admin',
    });
  expect(status).toBe(201);
  expect(typeof body).toBe('object');
  expect(typeof body.user).toBe('object');
...
test('POST /users 400 (master) - invalid role', async () => {
  const { status, body } = await request(app())
    .post(apiRoot)
    .send({
      access_token: masterKey, email: 'd@d.com', password: '123456', role: 'invalid',
    });
  expect(status).toBe(400);
  expect(typeof body).toBe('object');
